  ...
  body.forEach((row) => {
    ...
    Object.keys(row).forEach((p) => {
      if (p.indexOf(' ') !== -1 || p.indexOf('-') !== -1) {
        const camelized = undashCamelize(p);
        const v = row[p];
        delete row[p];
        row[camelized] = v;
      }
...
function filterBody(body, filter, colKeys, cumulative = false) {
  ...
  if (typeof filter === 'function') {
    ...
    const colKey = colKeys;
    return body.forEach((row) => {
      assertShowReactive(row);
      const val = getDataFromDotNotation(colKey, row);
      if (filter(val)) {
    ...
    return body.forEach((row) => {
      ...
      function someColumnContainsFilter() {
        return colKeys.some((col) => {
          const val = getDataFromDotNotation(col, row);
          const lowerCaseFilter = filter.toLowerCase();
          const columnIsStringAndContainsFilter = typeof val === 'string' && val.toLowerCase().indexOf(lowerCaseFilter) !== -1;
          const columnIsNumberAndContainsFilterAsAString = typeof val === 'number' && String(val).indexOf(filter) !== -1;
...
function sortBody(body, colKey, desc, compareFunction) {
  function numericCompare(row1, row2) {
    const valA = getDataFromDotNotation(colKey, row1);
    const valB = getDataFromDotNotation(colKey, row2);
  ...
  function lexicographicCompare(row1, row2) {
    const valA = String(getDataFromDotNotation(colKey, row1));
    const valB = String(getDataFromDotNotation(colKey, row2));
    if (valA === undefined || valB === undefined) {
      return 0;
    }
    const r = (valA > valB) ? 1 : -1;
    return r * (desc ? -1 : 1);
  }
  ...
  if (compareFunction !== undefined && typeof compareFunction === 'string') {
    ...
    const everyRowIsNonNumeric = body.every(r => !isNumeric(getDataFromDotNotation(colKey, r)));
