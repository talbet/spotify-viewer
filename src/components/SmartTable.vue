...
<div class="smart-table table-responsive">
  <table :class="tableClassesProcessed">
    <thead>
      <tr>
        <th v-for="col in tableHeader" class="headerClass" @click="doOrderBy(col.key)" track-by="key">
          {{col.label || col.key}}
        </th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="row in pBody" :id="rowId" track-by="_id" :class="trClasses(row._id, row._show)">
        <td v-for="col in tableHeader" track-by="key" :id="'cell-' + row._id + '-' + col.key" :class="tdClasses(col.key, row._id) + ' value-cell cell-' + col.key">
        </td>
      </tr>
    </tbody>
  </table>
...
import bodyParsing from '../utils/bodyParsing';
...
export default {
  ...
  computed: {
    headerClass() {
      const order = this.canOrderBy(this.col.key) ? 'ord' : '';
      return `col-${this.col.key} col-cell ${order} ${orderClass(this.col.key)}`;
    },
    rowId() {
      return `row-${this.row._id}`;
    },
    placeholder() {
      if (this.autoLoad) {
        return '...';
  ...
  beforeCompile() {
    ...
    this.elHarvest.forEach((el) => {
      ...
      const col = camelCase(el.attributes.col.value.replace(/\-/, ' ')
      ...
      const res = this.$root.$options.components[tag] ||
  ...
  methods: {
    makepBody(immediateRender = false) {
      ...
      this.body.forEach((row) => {
        malleableBody.push(Object.assign({}, row, {
          _show: true,
        }));
      });
      ...
      const insistingIds = this.pBody.map(r => r._id);
      const childrenToRemove = this.$children.filter(c => insistingIds.indexOf(c.id) === -1);
    ...
    compareFunction(sortKey) {
      const child = this.$children.find(c => c.col === sortKey);
    ...
    updateInjectedValues() {
      ...
      Array.from(this.$el.querySelectorAll('.value-cell')).forEach((cellEl) => {
        ...
        if (comp === undefined) {
          ...
          const dataMixin = {
            data() {
              const defaultValue = this.constructor.options.data ? this.constructor.options.data().value : undefined;
              return {
          ...
          const smartMethods = {
            methods: {
              enterEditMode() {
                this.$dispatch('enterEditMode', {
                  id: this.id,
                  col: this.col,
                });
              },
              saveNewValue() {
                this.$dispatch('saveNewValue', {
                  id: this.id,
                  col: this.col,
                });
              },
          ...
          const altCol = camelCase(col.replace(/\-/, ' ')
            .replace(/\./, 'ʬ').replace(/\+/, 'ʭ'))
            .replace(/ʬ/, '.').replace(/ʭ/, '+');
          const res = smartTable.column2stampMap[col] ||
          smartTable.column2stampMap[altCol];
          const elInstance = res ? res.el : document.createElement('plain-text');
          const compOptions = Object.assign({
            el: elInstance,
            mixins: [dataMixin, smartMethods],
            parent: smartTable,
          });
          const Ctor = res ? res.Ctor : Vue.extend(PlainText);
    ...
    put(resource, httpRESTreq = true) {
      this.$dispatch('before-request');
      const child = this.$children.find(c => c.id === resource.id && c.col === resource.col);
      if (child === undefined) {
    ...
    applyFilters() {
      ...
      this.filters.forEach((f) => {
        ...
        if (typeof col === 'string' || col.length === 1 && Array.isArray(col)) {
          const comp = this.$children.find(c => c.col === col);
  ...
  events: {
    saveNewValue({ id, col }) {
      const child = this.$children.find(c => c.id === id && c.col === col);
      if (id === '____add-row') {
    ...
    filter({ filter, col }) {
      ...
      const currentFilter = this.filters.find(f => sameCols(f.col, col));
